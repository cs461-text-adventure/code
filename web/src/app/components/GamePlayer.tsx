...
import { Game, Player, Room } from "@/lib/engine";

export default function GamePlayer({ gameData }: { gameData: any }) {
    ...
    useEffect(() => {
        if (gameData) {
            ...
            gameData.rooms.forEach((roomData: any) => {
                const room = new Room(roomData.description, {}, roomData.inventory);
            ...
            gameData.rooms.forEach((roomData: any) => {
                ...
                if (room) {
                    for (const [direction, connectedRoomId] of Object.entries(roomData.connections)) {
                        const connectedRoom = roomMap.get(connectedRoomId as string);
                        if (connectedRoom) {
                    ...
                    {output.map((interaction, i) => (
                        <div key={i} className="mb-4">
                            {interaction.map((msg, j) => (
                                <p key={j} className="mb-1" ref={i === output.length - 1 && j === interaction.length - 1 ? lastMessageRef : null}>
                                    {msg}
                                </p>
                            ))}
                        </div>
