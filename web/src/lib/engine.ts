...
export class Room {
    ...
    constructor(description: string, connections: { [key: string]: Room }, inventory: Item[]) {
...
export class Game {
    ...
    processCommand(input: string): string[] {
        ...
        const command = commands.includes(sanitizedInput as Command) ? (sanitizedInput as Command) : null;
        ...
        switch (command) {
            case "help":
                responses.push("Available commands: north, south, east, west, grab, attack, examine");
                break;
            case "north":
            case "south":
            case "east":
    ...
    processItemInput(input: string): string[] {
        ...
        const item = this._player.currentRoom.inventory.find(item => item.name.toLowerCase() === itemName);
        if (item) {
    ...
    processAttackInput(input: string): string[] {return [""]}
    processExamineInput(input: string): string[] {return [""]}
...
let testGame = {
    "rooms": [
        ...
        },
        {
            ...
            "name": "Hall",
            "description": "A grand corridor with high ceilings and an elegant chandelier casting flickering light over polished marble floors. Ornate portraits of unknown figures watch from the walls, their eyes seeming to follow your every move. The air is thick with a lingering scent of candle wax and aged wood.",
            "connections": {
        ...
        },
        {
            ...
            "name": "Lounge",
            "description": "A lavish room designed for comfort, with plush velvet armchairs and a crackling fireplace illuminating the space with a golden glow. A grand piano stands in the corner, its keys long untouched. The scent of old cigars and brandy still lingers in the air, remnants of past conversations.",
            "connections": {
        ...
        },
        {
            ...
            "name": "Library",
            "description": "Rows upon rows of towering bookshelves create a maze of forgotten knowledge. The scent of parchment and dust fills the air. A rolling ladder leans against one of the cases, allowing access to books high above. Dim candle sconces provide just enough light to read by, casting long shadows along the walls.",
            "connections": {
        ...
        },
        {
            ...
            "name": "Dining Room",
            "description": "A long mahogany dining table dominates the room, surrounded by high-backed chairs with intricate carvings. A chandelier hangs above, though its candles are barely flickering. The plates and silverware remain set, as if awaiting a feast that never arrived. A faint chill lingers in the air.",
            "connections": {
        ...
        },
        {
            ...
            "name": "Billiard Room",
            "description": "A cozy space with green felt-covered tables and a rack of well-worn cues mounted on the wall. A few half-empty glasses sit abandoned on a small side table, as if the last game ended in haste. A single, flickering lightbulb above casts eerie shadows across the room.",
            "connections": {
        ...
        },
        {
            ...
            "name": "Ball Room",
            "description": "A vast and empty space, its once-glorious parquet floor now scuffed with age. The towering windows let in beams of pale light, reflecting off an enormous crystal chandelier above. A faded velvet curtain conceals a small stage, where musicians once played for elegant dances long past.",
            "connections": {
        ...
        },
        {
            ...
            "name": "Kitchen",
            "description": "A cold, utilitarian space with long steel countertops and a massive iron stove. The scent of stale herbs and burnt wood still lingers. Copper pots hang from hooks, their surfaces dulled by time. A door to the pantry stands ajar, revealing shelves lined with jars of preserved goods, now covered in dust.",
            "connections": {
